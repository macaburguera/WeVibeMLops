...
def download_data(gdrive_link: str, raw_data_dir: str):
    ...
    if os.path.exists(zip_path):
        ...
        print(
            f"Decompressed images.zip to {os.path.join(raw_data_dir, 'images')}"
        )
...
def preprocess_images_in_batches(
    images_dir: str, labels: pd.DataFrame, transform, batch_size: int
):
    ...
    for image, target in image_generator():
        ...
        batch_targets.append(
            int(target)
        )  # Convert target to integer before appending

        if len(batch_images) == batch_size:
            yield torch.stack(batch_images), torch.tensor(
                batch_targets, dtype=torch.long
            )
...
def combine_batches_and_save(
    image_batches, target_batches, output_images_path, output_targets_path
):
    """Combine all batches and save them to disk."""
    if not image_batches or not target_batches:
        print(
            f"Warning: No batches to combine for {output_images_path} and {output_targets_path}."
        )
        return
...
def split_data(
    ...
    labels["breed"] = (
        labels["breed"].astype("category").cat.codes
    )  # Encode breeds as numeric values

    # Debug: Check for invalid labels
    num_classes = labels["breed"].nunique()
    assert labels["breed"].min() >= 0, "Labels must be >= 0"
    assert (
        labels["breed"].max() < num_classes
    ), f"Labels must be < {num_classes}"

    # Check for minimum class size
    min_class_size = labels["breed"].value_counts().min()
    if min_class_size < 2:
        raise ValueError(
            f"Each class must have at least 2 samples. Found minimum class size: {min_class_size}."
        )

    # Split into train, validation, and test sets
    train_labels, temp_labels = train_test_split(
        labels, test_size=0.3, stratify=labels["breed"], random_state=42
    )
    val_labels, test_labels = train_test_split(
        temp_labels,
        test_size=0.5,
        stratify=temp_labels["breed"],
        random_state=42,
    )
    ...
    for images, targets in preprocess_images_in_batches(
        images_dir, train_labels, transform, batch_size
    ):
    ...
    for images, targets in preprocess_images_in_batches(
        images_dir, val_labels, transform, batch_size
    ):
    ...
    combine_batches_and_save(
        ...
        os.path.join(
            processed_data_dir, "validation", "validation_targets.pt"
        ),
    )
    ...
    for images, targets in preprocess_images_in_batches(
        images_dir, test_labels, transform, batch_size
    ):
