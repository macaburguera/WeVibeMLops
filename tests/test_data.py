import os
import pytest
from src.dog_breed_classifier.data import (
...
def mock_labels(tmp_path):
    ...
    data = {
        ...
        "breed": ["breed1"] * 6
        + ["breed2"] * 6,  # At least 6 samples per class
    }
...
def test_preprocess_images_in_batches(mock_labels, mock_images):
    ...
    batches = list(
        preprocess_images_in_batches(
            str(mock_images), labels, transform, batch_size=2
        )
    )

    expected_batches = (
        len(labels) // 2
    )  # Calculate expected number of batches dynamically
    assert (
        len(batches) == expected_batches
    ), f"Expected {expected_batches} batches, but got {len(batches)}."
    assert batches[0][0].shape == (
        2,
        3,
        224,
        224,
    ), "Each batch should have correct dimensions."
    assert batches[0][1].shape == (
        2,
    ), "Batch targets should match batch size."
...
def test_split_data(mock_labels, mock_images, tmp_path):
    ...
    for subset in ["train", "validation", "test"]:
        ...
        assert (
            subset_dir / f"{subset}_images.pt"
        ).exists(), f"{subset}_images.pt should be created."
        assert (
            subset_dir / f"{subset}_targets.pt"
        ).exists(), f"{subset}_targets.pt should be created."
